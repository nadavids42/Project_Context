...
def render_privacy_banner() -> None:
    ...
    st.warning(
        ...
        "(Drive/Gmail/Calendar/Fathom) only discovers, downloads, parses, and "
        "stores evidence — it never sends anything to an LLM, even when an API "
        "key is configured. Only when you separately click **Extract "
        "observations** (or generate a brief) is that chunk's text sent to "
